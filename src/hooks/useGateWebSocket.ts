...
export const useGateWebSocket = (gateId: string) => {
  ...
  const handleWebSocketMessage = useCallback(
    (message: { type: string; payload: ZoneUpdateMessage }) => {
      if (!wsClient) return;
      ...
      if (message.type === "zone-update") {
        ...
        setZones((prevZones) => {
          ...
          const isOpen =
            payload.open !== undefined ? payload.open : existingZone.open;
          ...
          const updatedZone: Zone = {
            ...
            ...(payload.rateNormal !== undefined && {
              rateNormal: payload.rateNormal,
            }),
            ...(payload.rateSpecial !== undefined && {
              rateSpecial: payload.rateSpecial,
            }),
            availableSlots:
              payload.availableForVisitors ?? existingZone.availableForVisitors,
  ...
  useEffect(() => {
      ...
      try {
        if (msg.type === "zone-update" || msg.type === "admin-update") {
          handleWebSocketMessage(
            msg as { type: string; payload: ZoneUpdateMessage }
          );
        }
      } catch (err) {
    ...
    return () => {
      ...
      const clientWithRemove = wsClient as WebSocketClient & {
        removeEventListener?: (
